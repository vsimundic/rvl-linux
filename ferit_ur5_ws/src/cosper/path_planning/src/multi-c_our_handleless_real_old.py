...
if __name__ == '__main__':
	...
	try:
		...
		rospy.logerr('Cannot start robot commander. Aborting...')
		exit()

	T_G_0_vertices = np.zeros((2, 4, 4), dtype=np.float32)
	T_G_T = np.load(os.path.join(pkg_path, 'config/T_G_T_pen.npy'))
	T_G_T[:3, 3] = np.array([0., 0., 0.317])

	T_RP_TCP = np.eye(4)
	T_RP_TCP[:3, 3] = np.array([0.0775, 0., 0.098])
	T_TCP_T = np.eye(4)
	...
	T_G_0_array, q = path_planner.path2(np.array(q_init), -90.0, 37, False)
