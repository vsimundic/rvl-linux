...
from core.gazebo import get_link_pose, reset_gazebo_simulation, reset_gazebo_and_sync_moveit, reset_gazebo_and_robot
...
if __name__ == '__main__':
    ...
    for i in data.index.values.tolist():
        ...
        try:
            ...
            # RVL path planning
            T_G_0_array, q, all_feasible_paths, all_feasible_paths_q = rvl_path_planning(rvl_cfg, T_R_W, q_init, 17, False, cabinet_model, state_angle)
            ...
            if T_G_0_array.shape[0] > 1:
                ...
                # Reset Gazebo world and Moveit joints
                cabinet_model.delete_model_gazebo()
                # robot.send_joint_values_to_robot(q_init_ros.tolist(), wait=True)
                # reset_gazebo_and_sync_moveit('robot', robot.get_joint_names(), q_init_ros.tolist())
                robot.send_joint_values_to_robot(q_init_ros.tolist(), wait=True)
                reset_gazebo_and_robot('robot', robot.get_joint_names(), q_init_ros.tolist())
                rospy.sleep(5)
                robot.cancel_active_moveit_goal()
                robot.send_joint_values_to_robot(q_init_ros.tolist(), wait=True)
                robot.send_joint_values_to_robot(q_init_ros.tolist(), wait=True)
                robot.send_joint_values_to_robot(q_init_ros.tolist(), wait=True)
                rospy.sleep(2)

                # robot.reset_moveit_joints(q_init_ros.tolist())
