...
if __name__ == '__main__':
    ...
    cfg_path = '/home/RVLuser/ferit_ur5_ws/src/cosper/push_simulation/config/simulations_exp1_axis_left.yaml'
    config = read_config(cfg_path)
    csv_path = '/home/RVLuser/ferit_ur5_ws/src/cosper/push_simulation/simulation_results_exp1.csv'
    ...
    save_screenshot_path = '/home/RVLuser/ferit_ur5_ws/src/cosper/push_simulation/image_res_exp1'
    ...
    for i in range(doors.shape[0]):
        ...
        if T_G_0_array.shape[0] > 1:
            ...
            # feasible_path = all_feasible_paths[0, :, :, :]

            if i != 19:
                continue
            
            # Generate poses from the first state (opened by a state_angle value)
            T_G_S_unlatched = T_B_S @ feasible_path[3]
            ...
            T_G_0_path[-n_poses:, :, :] = np.linalg.inv(T_B_S)[np.newaxis, :, :] @ T_G_S_path
            ...
            execute_time = 20
            # trajectory_successful = robot.send_multiple_joint_space_poses_to_robot(q_path, execute_time, wait=True)
