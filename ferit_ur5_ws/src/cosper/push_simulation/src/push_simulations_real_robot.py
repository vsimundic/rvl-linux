...
if __name__ == '__main__':
    ...
    doors = np.load('/home/RVLuser/ferit_ur5_ws/src/cosper/push_simulation/config/door_params_poses.npy')

    # doors = doors[500:600, :]

    with open('/home/RVLuser/ferit_ur5_ws/src/cosper/push_simulation/simulation_results.csv', 'w') as f:
        ...
        # path found, traj_success, contact detected, door opened, width, height, x, y, z, rot_z, axis_pos
        writer.writerow(['path_found', 'traj_success', 'contact_free', 'door_opened', 'door_width', 'door_height', 'x', 'y', 'z', 'rot_z', 'axis_pos'])
    ...
    for i in range(doors.shape[0]):
        ...
        print('\n')
        T_A_S = np.eye(4)
        T_A_S[:3, 3] = np.array(doors[i, 2:5])
        T_A_S[2, 3] += T_B_S[2, 3]
        Tz_init = np.eye(4)
        Tz_init[:3, :3] = rot_z(np.radians(90.))
        T_A_S = T_A_S @ Tz_init
        Tz = np.eye(4)
        Tz[:3, :3] = rot_z(np.radians(doors[i, 5]))
        T_A_S = T_A_S @ Tz

        # Create a cabinet object
        cabinet_model = Cabinet(door_params=np.array([doors[i,0], doors[i,1], 0.018, 0.4]), 
                                axis_pos=-1,
                                T_A_S=T_A_S,
        ...
        if T_G_0_array.shape[0] == 1:
            ...
            with open('/home/RVLuser/ferit_ur5_ws/src/cosper/push_simulation/simulation_results.csv', 'a') as f:
                writer = csv.writer(f, delimiter=',')
                # success, width, height, x, y, z, rot_z
                # writer.writerow([final_success, door_params[0], door_params[1], cabinet_position[0], cabinet_position[1], cabinet_position[2], rotz_deg])
                
                # path found, traj_success, contact detected, door opened, width, height, x, y, z, rot_z
                writer.writerow([path_found, trajectory_successful, contact_free, door_opened, doors[i, 0], doors[i, 1], doors[i, 2], doors[i, 3], doors[i, 4], doors[i, 5], doors[i, 6]])
