...
void CRVLAImage::CreateBorder()
{
	...
	RVLAPIX *pAI;
	RVLAPIX *pAIEnd = m_pPix + pROI->right;

	for(pAI = m_pPix + pROI->left; pAI <= pAIEnd; pAI++)
		pAI->Flags |= RVLAPIX_FLAG_BORDER;

	int v;
	RVLAPIX *pRow = m_pPix + m_Width;

	for(v = pROI->top + 1; v < pROI->bottom; v++)
	{
		pAI = pRow + pROI->left;
		pAI->Flags |= RVLAPIX_FLAG_BORDER;
		pAI = pRow + pROI->right;
	...
	pAIEnd = pRow + pROI->right;

	for(pAI = pRow + pROI->left; pAI <= pAIEnd; pAI++)
		pAI->Flags |= RVLAPIX_FLAG_BORDER;
}
...
void CRVLAImage::Create(unsigned char *PixArray)
{
	...
	for(v = 0; v < m_Height; v++)
		for(u = 0; u < m_Width; u++, pPix++, pAPix++)
		{
			pAPix->u = (u << 1) + 1;
			pAPix->v = (v << 1) + 1;
			pAPix->I = (BYTE)(*pPix);
...
void CRVLAImage::Clear()
{
	...
	if(m_pPix)
		memset(m_pPix, 0, m_Width * m_Height * sizeof(RVLAPIX));
}
