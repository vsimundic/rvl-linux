...
void RunSeg2Bench(bool save)
{
	...
	if (save)
	{
		GenerateSSF(&surfels, ssfFileName);
	}
	...
	DrawSurfelImgAdjacencyOpenCV(segLabImg, &surfels);
	...
	RenderSurfelImgAdjacencyVTK(visualizer.renderer, &surfels);
	visualizer.Run();
}
