...
class push():
    ...
    def collision_detection_fcl(self, tool_mesh, tool_poses, cabinet_mesh, collision):
        ...
        # Cabinet: build BVH once at identity.
        fcl_cabinet_mesh = load_fcl_mesh_from_mesh(cabinet_mesh)
        cabinet_col_obj = create_collision_object(fcl_cabinet_mesh, np.eye(4))

        # Tool: build BVH once, reuse by updating the transform each iteration.
        ...
        # Reuse a single request/result pair.
        request = fcl.CollisionRequest()

        collision_results = []
        for T_G_DD in tqdm(tool_poses):
            # Update only the transform — no BVH rebuild.
...
def demo_push_poses():
    ...
    if use_default_gripper:
        ...
        if use_one_finger:
            ...
            tool_finger_distances = [0.072623, 0., -0.205678] # x, y, z # 0.055 is tactile sensor
            # tool_finger_distances = [-0.072462, -0.205678, 0.0] # x, y, z # 0.055 is tactile sensor
    ...
    if visualize_feasible_poses:
        ...
        # Build TCS prism wireframe in TCP frame.
        # params[0] and params[2] are the two top edges (front and back), params[1] is the shared bottom edge.
        # Each is mirrored in y to get both sides of the prism:
        #   v0 = params[0]          (front top, +y)
        #   v1 = params[0] y-mirror (front top, -y)
        #   v2 = params[1]          (bottom,    +y)  -- shared by both faces
        #   v3 = params[1] y-mirror (bottom,    -y)  -- shared by both faces
        #   v4 = params[2]          (back top,  +y)
        #   v5 = params[2] y-mirror (back top,  -y)
        tcs_params = tool.tool_contact_surface_params
        tcs_verts_TCP = np.stack((
            tcs_params[0,:],                           # v0
            tcs_params[0,:] * np.array([1, -1, 1]),    # v1
            tcs_params[1,:],                           # v2
            tcs_params[1,:] * np.array([1, -1, 1]),    # v3
            tcs_params[2,:],                           # v4
            tcs_params[2,:] * np.array([1, -1, 1]),    # v5
        ))
        tcs_edges = np.array([
            [0, 1],   # front top edge
            [2, 3],   # bottom edge
            ...
            [4, 2],   # back +y side
            [5, 3],   # back -y side
            [0, 4],   # top +y connecting edge (front to back)
            [1, 5],   # top -y connecting edge (front to back)
        ])
        ...
        def update_tool_mesh(vis):
            ...
            T_G_W = door.T_DD_W @ T_G_DD
            # Tool origin frame
            tool_origin_rf = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1)
            tool_origin_rf.transform(T_G_W)
            vis.add_geometry(tool_origin_rf, reset_bounding_box=False)
            ...
            # TCP sphere
            tcp_sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.0005)
            tcp_sphere.translate(T_TCP_W[:3, 3])
            tcp_sphere.paint_uniform_color([1.0, 0.0, 0.0])  # Red
            vis.add_geometry(tcp_sphere, reset_bounding_box=False)
